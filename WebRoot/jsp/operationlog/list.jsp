...
<body class="main_body">
	...
	<div class="main_search">
		...
		<div class="main_table">
			...
			<div class="main_search">
				<form id="page_form"
					action="<%=basePath%>operationlog/findPageOperationLogs.do">
						...
						<tr>
							<td style="text-align: left;">
								...
								<div class="search_btn">
									<input type="button" id="clearLogId" value="查询"
										onclick="query()" style='margin-right: 50px' />
								</div>
							</td>
						</tr>
					</table>
					...
					<div style="height: 640px; margin-left: 5px;" id="logdiv"
						name="logdiv">
						<div style="height: 640px; overflow-y: auto; max-height: 640px;"
							id="logdiv" name="logdiv">
							<table id="logTable" name="logTable" cellpadding="0"
								cellspacing="0" class="table" width="850px"
								style="table-layout: fixed">
								<c:forEach var="m" items="${logList}" varStatus="index">
									<tr>
										<td width="100px">${m.username}</td>
										<td width="100px">${m.ip}</td>
										<td width="100px" style="word-wrap: break-word;">
											${m.operation}</td>
										<td width="100px">${m.result}</td>
										<td width="250px" style="word-wrap: break-word;">
											${m.content}</td>
										<td width="150px"><fmt:parseDate value="${m.createtime}"
												pattern="yyyy-MM-dd HH:mm:ss" var="showCreateTime" />
											                             <fmt:formatDate
												value="${showCreateTime}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
									</tr>
								</c:forEach>
							</table>
						</div>
