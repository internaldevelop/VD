...
<script type="text/javascript"
	src="<%=request.getContextPath() %>/jsp/testexecute/js/list.js"></script>
<script type="text/javascript"
	src="<%=request.getContextPath() %>/commons/js/socketfun.js"></script>
<script
	src="<%=request.getContextPath()%>/jsp/testresult/js/highstock.src.js"></script>
...
<script
	src="<%=request.getContextPath()%>/jsp/testresult/js/offline-exporting.js"></script>
<script
	src="<%=request.getContextPath()%>/jsp/testresult/js/grid-light.js"></script>
<script
	src="<%=request.getContextPath()%>/jsp/testresult/js/highcharts-zh_CN.js"></script>

<script type="text/javascript"
	src="<%=request.getContextPath() %>/jsp/testexecute/js/chart.js"></script>
<style type="text/css">
.hitory {
	position: absolute;
	right: 150px;
...
.tabs-header .tabs-wrap ul.tabs {
	float: none;
}

.tabs-header .tabs-wrap ul.tabs li {
	float: none;
...
.tabs-header .tabs-wrap ul.tabs li.tabs-selected {
	background: white;
	z-index: 1;
}
...
.tabs-panels .tab-panel.tabs-selected {
	background: white;
	display: block;
}
</style>
	...
	<div class="index_body02">
		...
		<div class="main_table">
			...
			<div class="main_search">
				<form name="form1" id="form1" class="demoform"
					action="<%=request.getContextPath() %>/testexecute/findTestSuite.do"
					method="post">
					<table width="100%" style="table-layout: fixed;" border=0>
						<tr>
							<td style="width: 60px;" class="search_word">选择模板:</td>
							<td style="width: 300px;" class="search_input"><select
								name="tempId" id="tempId" onchange="fun_change(this);">
									<option value="0">请选择</option>
									<c:forEach var="testTemp" items="${list}" varStatus="index">
										<option value="${testTemp.id }"
											<c:if test="${testTemp.id==tempId }">
								selected
							</c:if>>${testTemp.name }</option>
									</c:forEach>
							</select></td>
			...
			<div class="fix_table main_search" id="fix_div">
				...
				<table cellpadding="0" cellspacing="0" id="testSuiteTable"
					class="table">
					<%
       //选择的监视器
       ...
       %>
					<tr align='center' bgcolor="#FFFFFF" height="26">
						<td width="100px"><a class='<%=tce.getCssImg() %>'
							href='javaScript:void(0)' onclick='startSingleTest(this,<%=i %>)'></a>
						</td>
						<td><%=tce.getName() %></td>
						<%
				if(exeList!=null){
				...
				%>
						<td width="120px" class="<%=m.get("ID") %>">
							<%if("4".equals(s)){%> <img src="../images/ya.png"> <%}else if("5".equals(s)){%>
							<img src="../images/no.png"> <%}else if("1".equals(s) || "2".equals(s) || "3".equals(s)){%>
       	...
       	%>
			...
			<div class="main_search">
				<table width="100%" style="table-layout: fixed;" border=0>
					<tr>
						<td style="width: 200px;" class="search_btn_left"><input
							type="button" value="全部执行" id="btn_start" onclick="startTest();" />
							<input type="button" value="停    止" id="btn_stop"
							onclick="stopTest();" /> <span id="info"></span></td>
						<td class="search_btn_left">
							...
							<span id="logProgressDisplay" name="logProgressDisplay"
			...
			<div class="main_search" style="padding-left: 3px">
						...
						<td style="text-align: left;">
							<form id="chkForm">
								<c:forEach var="m" items="${monitorList}" varStatus="index">
									<input type='checkbox' name="chk_m" id="chk_${m.id}"
										value="${m.id }" style='margin-left: 20px'
										onclick="checkChart(this)"
										<c:if test="${m.checked==true}" >checked</c:if>>${m.name }
				</c:forEach>
							</form>
						</td>
					</tr>
            	...
            	%>
							<div class="chartDiv" id="<%=m.get("id") %>"
								<%=(Boolean)m.get("checked")?"":"style='display:none'" %>>
							</div> <%
            	...
            	%>
							<div class="chartDiv" id="<%=m.get("id") %>_1"
								<%=(Boolean)m.get("checked")?"":"style='display:none'" %>>
							</div>
							<div class="chartDiv" id="<%=m.get("id") %>_2"
								<%=(Boolean)m.get("checked")?"":"style='display:none'" %>>
							</div> <%
            	}
            	}
			...
			<div class="table_title">
				...
				<div class="search_input">
					<input id="hitory" class="hitory" type="button" value="历史回顾"
						id="btn_history" onclick="hitory();">
				</div>
				<div class="search_input">
					<input id="hitory" class="closehitory" type="button" value="收起回顾"
						id="close_history" onclick="closeHitory();">
				</div>
			</div>
							...
							<li>
								<ul class="search_ul">
									...
									<li class="search_input"><input type='text'
										...
										onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',maxDate:'#F{$dp.$D(\'endDate\');}'})" /></li>
									<%-- ,maxDate:'#F{$dp.$D(\'endDate\');}' minDate:'%y-%M-%d #{%H-24}:%m:%s'
							--%>
								</ul>
							</li>
							<li>
			...
			<div class="main_search historyBox" style="padding-left: 3px">
				...
				<div class="tabs-panels">
					<div class="historyChartDiv tab-panel tabs-selected"
						id="arpcontainer"></div>
					<div class="historyChartDiv tab-panel" id="icmpcontainer"></div>
					<div class="historyChartDiv tab-panel" id="tcpcontainer"></div>
						...
						<tr>
							<td colspan=4 style="padding-left: 10px; text-align: left;">
								...
								<c:forEach var="m" items="${logList}" varStatus="index">
									<input type='checkbox' name="ms" value="${m.id }"
										onclick="checkLog(this)" style='margin-left: 20px'
										<c:if test="${m.checked==true}" >checked</c:if>>${m.name }
				</c:forEach>
							</td>
							<td class="search_btn"><input type="button" id="clear"
								value="清除" onclick="clearLog()" style='margin-right: 50px' /></td>
						</tr>
						<tr>
							<td colspan=5 class="search_input">
								...
								<div class="fix_table" id="logdiv" name="logdiv">
									<table id="logTable" name="logTable" cellpadding="0"
										cellspacing="0" class="table" width="350px">
									</table>
								</div>
							</td>
						</tr>
