...
<script type="text/javascript"
	src="<%=request.getContextPath() %>/jsp/portscan/js/list.js"></script>
<script type="text/javascript"
...
.main_table table td a.asc {
	background-image: url("${basePath}/images/icon_num_top.png");
	background-repeat: no-repeat;
}

.main_table table td a.desc {
	background-image: url("${basePath}/images/icon_num_bot.png");
	background-repeat: no-repeat;
}
...
<body class="main_body">
	<div class="index_body02">
		...
		<div class="main_table">
			...
			<!--middle search-->
			<input type="hidden" value="${scanport.startScan }"
				id="startScanFlag" />
			<div class="main_search">
				<form name="form1" id="form1" class="demoform" method="post"
					action="<%=request.getContextPath() %>/portscan/startScan.do">
					<table width="100%" style="table-layout: fixed;" border=0>
						<tr>
							...
							<td class="search_btn"><span id="msgdemo" name="sa"></span>
								<input type="submit" id="btn_ctr"
								value="<c:if test="${1==scanport.startScan}" >停止扫描</c:if><c:if test="${1!=scanport.startScan}" >开始扫描</c:if>" />

							</td>
						</tr>
						<tr>
							<td class="search_word">端口类型:</td>
							<td class="search_input"><select name="portType"
								id="portType">
									<option value="6"
										<c:if test="${6==scanport.portType}" >selected="selected"</c:if>>TCP</option>
									<option value="17"
										<c:if test="${17==scanport.portType}" >selected="selected"</c:if>>UDP</option>
							</select></td>
							<td class="search_word">扫描类型:</td>
							<td class="search_input"><select name="scanType"
								id="scanType" onchange="changeScanType();">
									<option value="0"
										<c:if test="${0==scanport.scanType}" >selected="selected"</c:if>>主动扫描</option>
									<option value="1"
										<c:if test="${1==scanport.scanType}" >selected="selected"</c:if>>被动扫描</option>
							</select></td>
			...
			<div class="main_search">
				<form name="form2" id="form2" class="demoform" method="post"
					action="<%=request.getContextPath() %>/portscan/addPort.do">
					<table width="100%" style="table-layout: fixed;" border=0>
						<tr>
							...
							<td style="width: 300px;" class="search_input"><select
								name="hportType" id="hportType">
									<option value="6">TCP</option>
									<option value="17">UDP</option>
							</select></td>
							<td class="search_btn"><input type="submit" value="添加"
								id="sumitBtn"
								<c:if test="${1==scanport.startScan}" >disabled="disabled"</c:if> />
								<span id="msgdemo"></span></td>
						</tr>
				...
				<div id="title_div" style="padding-right: 17px;">
					<table cellpadding="0" cellspacing="0" class="table">
						<tr class="table_title02">
							<td width="10%">端口<a id="sort" href="javascript:void(0);"
								class="asc"></a></td>
							<td width="35%">服务</td>
							<td width="15%">来源</td>
							<td width="15%">类型</td>
						...
						<c:forEach var="scan" items="${list}" varStatus="index">
							<tr>
								...
								<td width="10%"><a href="javascript:void(0)"
									onclick="deletePort('${scan.id }','${scan.PORTTYPEY }','${scan.PORTNUM }',this)">删除</a></td>
							</tr>
						</c:forEach>
					</table>
				</div>
