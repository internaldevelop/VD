...
<script type="text/javascript"
	src="<%=request.getContextPath() %>/jsp/dynamicports/js/list.js"></script>
<script type="text/javascript"
		...
		<div class="main_table">
			...
			<div class="main_search">
				<form name="form1" id="form1" class="demoform" method="post"
					action="<%=request.getContextPath() %>/dynamicports/startScan.do">
					<table width="98%" style="table-layout: fixed;" border=0>
						<tr>
							...
							<td style="width: 300px;" class="search_input">
								...
								<input type='text' name='ip' id='ip' value='${ipAddr}'
							...
							<td class="search_input"><select name="scanType"
								id="scanType">
									<option value="0"
										<c:if test="${0==scanType}" >selected="selected"</c:if>>主动扫描</option>
									<option value="1"
										<c:if test="${1==scanType}" >selected="selected"</c:if>>被动扫描</option>
							</select></td>
							...
							<td class="search_input"><select name="portType"
								id="portType">
									<c:forEach var="pt" items="${portTypes }">
										<option value="${pt.port }"
											${portType==pt.port?"selected":"" }>${pt.protocol }</option>
									</c:forEach>
							</select></td>
						</tr>
						<tr>
