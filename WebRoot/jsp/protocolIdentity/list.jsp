...
<script type="text/javascript"
	src="<%=request.getContextPath() %>/jsp/protocolIdentity/js/list.js"></script>
<script type="text/javascript"
	...
	<div class="index_body02">
		...
		<div class="main_table">
			<div class="table_title">
				<div>协议识别</div>
				<form name="form" id="form" class="demoform" method="post"
					action="<%=request.getContextPath() %>/protocol/startProtocolIdentity.do">
					<div class="search_input">
						<input type="hidden" value="${proMap.startIdentity }"
							id="startIdentityFlag" /> <input class="start" type="submit"
							id="btn_ctr"
							value="<c:if test="${1==proMap.startIdentity}" >停止识别</c:if><c:if test="${1!=proMap.startIdentity}" >开始识别</c:if>" />
					</div>
				</form>
			</div>
			<div style="width: 100%; float: left; height: 60%;">
				<div id="title_div" style="padding-right: 17px;">
					<table cellpadding="0" cellspacing="0" class="table">
						<tr class="table_title02">
							<td width="10%">序号<a id="sort" href="javascript:void(0);"
								class="asc"></a></td>
							<td width="15%">协议名称</td>
							<td width="15%">源IP</td>
							<td width="15%">目的IP</td>
				...
				<div class="" style="float: left; overflow-y: auto; width: 100%;"
					id="fix_div">
					<table id="protocolTable" cellpadding="0" cellspacing="0"
						class="table" style="">
						<c:forEach var="protocol" items="${list}" varStatus="index">
							<tr>
								...
								<td width="20%"><fmt:formatDate
										value="${protocol.IDENTYTIME }" pattern="yyyy-MM-dd HH:mm:ss" />&nbsp;</td>
								<td width="10%"><a href="#"
									onclick="deleteProtocol('${protocol.ID }',this)">删除</a></td>
							</tr>
						</c:forEach>
					</table>
				</div>
