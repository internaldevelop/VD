...
.main_table table.table td button {
	padding: 5px 15px;
	margin: 2px;
}
-->
...
<body class="main_body">
	<div class="index_body02">
		...
		<div class="main_table">
			...
			<form action="<%=request.getContextPath() %>/securityCheck/save.do"
				method="post">
				<table cellpadding="0" cellspacing="0" class="table">
					<tbody id="baselineTbody">
						<c:forEach var="bl" items="${list }">
							<tr>
								<td><label>选项名称：<input type="text" name="names"
										value="${bl.name }" /></label></td>
								<td><label>基线值：<input type="text" name="values"
										value="${bl.value }" /></label></td>
