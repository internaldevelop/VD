...
public class AckManager implements Disconnectable {
	...
	class AckEntry {
		...
		final Map<Long, AckCallback<?>> ackCallbacks = PlatformDependent
	...
	private final Map<UUID, AckEntry> ackEntries = PlatformDependent
			.newConcurrentHashMap();
	...
	public void onAck(SocketIOClient client, Packet packet) {
		AckSchedulerKey key = new AckSchedulerKey(Type.ACK_TIMEOUT,
				client.getSessionId(), packet.getAckId());
		scheduler.cancel(key);

		AckCallback callback = removeCallback(client.getSessionId(),
				packet.getAckId());
		if (callback == null) {
			return;
		}
		if (callback instanceof MultiTypeAckCallback) {
			callback.onSuccess(new MultiTypeArgs(packet
					.<List<Object>> getData()));
		} else {
			Object param = null;
			...
			if (args.size() > 1) {
				log.error( //zgh
						...
						packet.getAckId(), client.getSessionId());
	...
	public long registerAck(UUID sessionId, AckCallback callback) {
		...
		if (log.isDebugEnabled()) {
			log.debug("AckCallback registered with id: {} for client: {}",
					index, sessionId);
		}
	...
	private void scheduleTimeout(final long index, final UUID sessionId,
			AckCallback callback) {
		if (callback.getTimeout() == -1) {
			return;
		}
		SchedulerKey key = new AckSchedulerKey(Type.ACK_TIMEOUT, sessionId,
				index);
	...
	public void onDisconnect(ClientHead client) {
		...
		for (Long index : indexes) {
			...
			SchedulerKey key = new AckSchedulerKey(Type.ACK_TIMEOUT,
					client.getSessionId(), index);
			scheduler.cancel(key);
		}
	}
